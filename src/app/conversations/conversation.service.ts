...
export class ConversationService {
  ...
  handleNavigateToConversation(userPublicId: string): void {
    this.createOrLoadConversation$.next(userPublicId);
  }

  handleGetAll(pagination: Pagination): void {
    const params = createPaginationOption(pagination);
    this.http.get<Array<Conversation>>(`${environment.API_URL}/conversations`, {params})
      .subscribe({
        next: conversations => {
  ...
  private mapDateToDayJsList(conversations: Array<Conversation>): void {
    conversations.forEach(conversation => this.mapDateToDayJs(conversation));
  }

  private mapDateToDayJs(conversation: Conversation): void {
  ...
  private sortMessageByTime(conversations: Array<Conversation>): void {
  ...
  handleCreate(conversationToCreate: ConversationToCreate): void {
  ...
  handleDelete(conversationPublicId: string): void {
  ...
  handleGetOne(conversationId: string): void {
    const params = new HttpParams().set('conversationId', conversationId);
    this.http.get<Conversation>(`${environment.API_URL}/conversations/get-one-by-public-id`, {params})
      .subscribe({
        next: conversation => {
          this.mapDateToDayJs(conversation);
  ...
  navigateToNewConversation(conversation: Conversation): void {
