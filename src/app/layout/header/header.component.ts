...
export class HeaderComponent implements OnInit {
  ...
  private listenToDeleteConversation(): void {
    this.sseService.deleteConversation
      .pipe(filter(conversationPublicId => conversationPublicId === this.conversation?.publicId))
      .subscribe(() => {
  ...
  private pollUserPresence(): void {
    ...
    this.lastSeenSub = interval(5000)
      .subscribe(() => this.oauth2Service
        .handleLastSeen(this.userSelected?.publicId!));
  }

  private unsubscribeLastSeen(): void {
    if (this.lastSeenSub) {
